...
def main():
    ...
    unique_test_preds = np.unique(all_test_preds)

    print(f"Unique classes in true labels: {unique_test_labels}")
    print(f"Unique classes in predictions: {unique_test_preds}")
    ...
    try:
        ...
        # Specify the labels parameter if necessary
        # Generate all classes based on the label encoder's classes
        all_classes = list(range(len(label_encoder.classes_)))
        
        print("Generating classification report with specified labels.")
