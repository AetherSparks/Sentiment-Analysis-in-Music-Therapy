...
def main():
    ...
    unique_labels = np.unique(all_test_labels)  # Get unique labels from test data
    # Create a subset of target_names that matches unique labels
    target_names_subset = label_encoder.classes_[unique_labels] if unique_labels.max() < len(label_encoder.classes_) else label_encoder.classes_

    print(classification_report(
        all_test_labels, 
        all_test_preds, 
        target_names=target_names_subset,  # Use only the relevant target names
        zero_division=0
    ))
